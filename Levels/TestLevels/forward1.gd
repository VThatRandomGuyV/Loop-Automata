...
@onready var action_animations: AnimationPlayer = $"../ActionAnimations"
...
func _on_area_2d_input_event(viewport: Node, event: InputEvent, shape_idx: int) -> void:
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed:
			if forward.visible == true:
				mouse_offset = position - get_global_mouse_position()
				selected = true
				action_animations.play("ShakeForward")
				pick_up.play()
		else:
			selected = false
			hovering = false
			_set_position()
			action_animations.stop()
...
func _on_box_fwd_1_in() -> void:
	fwd_in = true
	fwd_out = false

func _on_box_fwd_1_out() -> void:
	fwd_out = true
	fwd_in = false
